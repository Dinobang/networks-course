...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   Ответ: Адрес: 192.168.1.44, Порт: 52267, а после уже 50538
   
   ![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/1.png)
   ![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/2.png)
   
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   TCP-сегментов этого соединения?
   
   Ответ: ip адресс  - 149.154.167.51
   номера портов - 443 у меня
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   Ответ: Номер, видимо, ноль. Там в скобочках подписано, какой это сегмент
   
   ![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/3.png)
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   Ответ: номер, видимо, снова нулевой. Значение - 0x012. Как определил - сервер поставил 1 на места SYN и ACK сегентов
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   Вбиваю http.request.method == "POST", пакет не находится, ни если в сафари, ни если в гугле( Руками поискала как написано, не нашла
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   Ответ: подозреваю, что нужно поделить window size на rtt средний, но у меня нет нужного пакета( 
...
#### Скрин
![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/4.png)
...
#### Демонстрация работы
![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/5.png)
...
#### Демонстрация работы
![Image alt](https://github.com/dinobang/networks-course/raw/master/lab07/images/6.png)
