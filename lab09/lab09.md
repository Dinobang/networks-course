...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - да, отличаются. наши выдают time exceed, падая с лимитом. у этих запросов есть порты, в отличие от прошлых
...
3. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - да, появились новые поля, вот они. в них информация о том, какое сообщение отправляется
     
5. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   отличаются от ICMP-пакетов, сообщающих об ошибках? Чем объясняются такие отличия?

   <img src="images/8.png" width=700 />
   
   - этот пакет не превысил время)) он просто не нашел нужный порт и не смог доставить информацию
   - тем, что мы пытаемся попасть не натот порт, возможно..
     
7. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - вот оно! да, превышает
   - я погуглила, кажется, это нью-йорк и что-то еще, не смогла определить, что такое pao(
