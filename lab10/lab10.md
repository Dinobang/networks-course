...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Ответ: TTL, identificator, header checksum
   3. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Ответ: не меняются - source, destination, protocol, ECN, total length, Flags, Fragment offset
   ...
   4. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Ответ: это поле увеличивается на 1!
...
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   ...
   - Ответ: identification = 33754, TTL = 64
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Ответ: у меня меняется только идентификатор
...
9. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      Ответ: попыталась поставить разные размеры, либо посылает одним пакетом без фрагментации, либо не отправляет вообще...
      
   3. Какие поля IP-заголовка изменяются в разных фрагментах?
      Так же как в прошлом вопросе
...
#### Демонстрация работы
В пункте Б фотографии с двух серверов - гугл и какой-то африканский...
