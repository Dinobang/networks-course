...
Чтобы получить среднюю пропускную способность, нам нужно поделить объем данных на время. А так как средний размер окна - 3$\dfrac{W}{4}$, значит средняя пропускная 3$\dfrac{W}{4}$ в RTT.
...
1. Судя по неваренствам, когда придет подтверждение от 2 пакета, 3 пакет уже отправится. А окно будет увеличено до 4х. На это тратим $\dfrac{𝑆}{𝑅}$ + RTT. Подтверждение за четвертый придет до конца отправки пакетов, так что окно будет увеличено до 8ми. Дальше такого происходить не будет и пакеты будут отправляться нормально. Получаемся: 3RTT + 12$\dfrac{𝑆}{𝑅}$
2. Здесь очень похожая картина, но мы получим после 3 пакета подтверждение за 2, а после седьмого за четвертый. И тогда дальше пакеты будут отправляться нормально. Получаемся: 4RTT + 11$\dfrac{𝑆}{𝑅}$
3. Теперь RTT маленький, то есть ответы будут доходить и пакеты отправляться нормально все время. Таким образом, 2RTT + 15$\dfrac{𝑆}{𝑅}$
...
Количество отправленных пакетов = $\frac{W}{2} \cdot (1 + (1 + \alpha) + ... + (1 + \alpha)^n)$. Если $n = log_{1 + \alpha}(2)$, то эта формула сворачивается в функциональную зависимость между частотой потерь и максимальным размером окна, так как количество пакетов теперь = $\frac{W}{2} (1 + \frac{1 + \alpha}{2\alpha})$, а частота потери это 1/количество пакетов. 
...
Я мб что-то не понимаю, но кажется, на общение и отправку пакетов нам нужно 4RTT, но только $RTT_{FE}$. Обмен данными происходит при необходимости за $RTT_{BE}$. Время обработки, кажется, менятся недолжно никак. Получилось то, что написано!
